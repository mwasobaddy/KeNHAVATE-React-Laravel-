...
export default function Edit({ directorate, regions }: Props) {
    const form = useForm({
        name: directorate.name,
        code: directorate.code,
        region_id: directorate.region_id.toString(),
    ...
    const onSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        form.patch(`/administration/directorates/${directorate.id}`, {
            onSuccess: () => {
                toast.success('Directorate updated successfully!');
            },
            onError: () => {
                toast.error('Failed to update directorate. Please check the form.');
            },
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${directorate.name}`} />
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">Edit Directorate</h1>
                        <p className="text-muted-foreground">
                            Update directorate details
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={onSubmit} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        <Button type="submit" disabled={form.processing}>
                            ...
                            {form.processing ? 'Updating...' : 'Update Directorate'}
                        </Button>
                    </div>
                </form>
            </div>
