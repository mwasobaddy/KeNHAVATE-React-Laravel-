        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true" aria-label={title} onKeyDown={onKeyDown} tabIndex={-1}>
            ...
            <div className="bg-white dark:bg-[#4c4843] rounded-lg p-6 z-10 w-full max-w-lg shadow-lg">
                ...
                <div className="mt-4 flex justify-end gap-2">
                    <button onClick={onCancel} className="px-3 py-2 rounded bg-gray-100 text-[#231F20] hover:bg-gray-200 transition-colors">{cancelLabel}</button>
                    <button onClick={onConfirm} className="px-3 py-2 rounded bg-orange-600 text-white shadow-sm hover:bg-orange-700 transition-colors">{confirmLabel}</button>
                </div>
            </div>
        </div>
    );
